...
vec4 compare(vec4 current, vec4 reference) {
...
void main() {
    ...
    //
    vec4 M = vec4(0); // (M_00, M_11, M_01, M_10)
    ...
    // Simple Matching Coefficient: (M_00 + M_11) / (M_00 + M_11 + M_01 + M_10)
    SMC = (M[0] + M[1]) / (M[0] + M[1] + M[2] + M[3]);
