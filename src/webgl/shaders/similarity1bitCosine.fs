...
float dotBinary(vec4 x, vec4 y) {
    ...
    uvec4 X = uvec4(round(x * 255.0));
    uvec4 Y = uvec4(round(y * 255.0));

    // Dot product of x and y represented as binary vectors of 32 bits.
    // => Number of bits set to 1 in both representations.
    return float(BIT_COUNT[X.r & Y.r] + BIT_COUNT[X.g & Y.g] + BIT_COUNT[X.b & Y.b] + BIT_COUNT[X.a & Y.a]);
}
...
void main() {
    ...
    for (int i = 0; i < <%=TILES=%>; i++) {
        ...
        currentLength += dotBinary(current, current);
        referenceLength += dotBinary(reference, reference);
        angle += dotBinary(current, reference);
    }
